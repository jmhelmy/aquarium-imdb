...
export default async function FishDetailPage({ params }: FishDetailPageProps) {
  ...
  // Fetch the fish from your database
  const fish = await prisma.fish.findUnique({
    where: { id: fishId },
  });

  // If no fish found, show 404
  if (!fish) {
    notFound();
  }
  ...
  return (
    <main className="min-h-screen bg-blue-50 py-10 px-6">
      <div className="max-w-4xl mx-auto">
        ...
        {/* Display the rest of your fields if they exist */}
        {fish.type && (
          <p className="mb-2 text-lg text-gray-700">Type: {fish.type}</p>
        )}
        {fish.size && (
          <p className="mb-2 text-lg text-gray-700">Size: {fish.size}</p>
        )}
        {fish.tankSize && (
        ...
        {fish.waterHardness && (
          <p className="mb-2 text-lg text-gray-700">
            Water Hardness: {fish.waterHardness}
          </p>
        )}
        {fish.swimLevel && (
          <p className="mb-2 text-lg text-gray-700">
            Swim Level: {fish.swimLevel}
          </p>
        )}
        {fish.aggression && (
          <p className="mb-2 text-lg text-gray-700">
        ...
        {fish.food && (
          <p className="mb-2 text-lg text-gray-700">Food: {fish.food}</p>
        )}
        {fish.compatibility && (
          <p className="mb-2 text-lg text-gray-700">
        ...
        {fish.origin && (
          <p className="mb-2 text-lg text-gray-700">Origin: {fish.origin}</p>
        )}
        {fish.colorVariants && (
          <p className="mb-2 text-lg text-gray-700">
            Color Variants: {fish.colorVariants}
          </p>
        )}
        {fish.careNotes && (
          <p className="mb-2 text-lg text-gray-700">
            Care Notes: {fish.careNotes}
          </p>
        )}
        {fish.notes && (
          <p className="mb-2 text-lg text-gray-700">Notes: {fish.notes}</p>
        )}
      </div>
    </main>
  );
